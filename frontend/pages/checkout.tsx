...
export default function Checkout() {
    ...
    if (raw) {
      ...
      if (existingRaw) {
        try {
          ...
          // ✅ SADECE AYNI PAKET VARSA ENGELLE
          const samePackageTrialExists = existing.some(
            (a: any) =>
              a.name === parsed.packageName &&
              a.status === "trial"
          );

          setAlreadyHasTrial(samePackageTrialExists);
        } catch {
          setAlreadyHasTrial(false);
        }
      }
    }
  }, []);
  ...
  const activateDemo = () => {
    ...
    const automation = {
      id: `auto_${now}`,
      name: data.packageName, // ✅ ARTIK AYIRT EDİCİ
    ...
    localStorage.setItem(
      "clever_active_automations",
      JSON.stringify([automation, ...existing])
    );
        ...
        {!activated ? (
          <Card className="p-8">
            ...
            <p className="text-white/60 mb-8 text-center">
              <strong>{data.packageName}</strong> için demo başlatıyorsunuz
            </p>
            ...
            {alreadyHasTrial ? (
              <div className="text-center p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl">
                <p className="text-yellow-400 text-sm">
                  Bu otomasyon için zaten aktif bir denemeniz var.
                </p>
